...
export default function CollectTable() {
  ...
  const columns = [
    ...
    },
    {
      title: "操作",
      key: "action",
      render: (_: unknown, record: Collect) => (
        <Space>
          <Popconfirm
            title="确定删除该收藏记录？"
            onConfirm={() => handleDelete(record.collectId)}
          >
